...
export default function BuyerLayout({ children }) {
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-slate-100 font-sans flex flex-col selection:bg-indigo-500 selection:text-white overflow-x-hidden w-full">
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between gap-2 sm:gap-4">
                    ...
                    <form onSubmit={handleSearchSubmit} className="hidden md:flex relative flex-1 max-w-lg mx-4">
                        <input
                            ...
                            className="w-full bg-slate-800/80 border border-slate-700 text-slate-100 rounded-full pl-5 pr-24 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all placeholder-slate-400 shadow-inner"
                        />
                        <button
                            type="submit"
                            className="absolute right-1 top-1 bottom-1 px-4 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white text-xs font-bold rounded-full transition-all flex items-center gap-1 shadow-md hover:shadow-indigo-500/25"
                        >
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 text-xs font-bold shrink-0">
                        ...
                        >
                            ...
                            <span className={`absolute -top-1 -right-1 bg-rose-600 text-white font-mono text-[9px] font-bold min-w-4 h-4 px-1 rounded-full flex items-center justify-center border border-slate-900 shadow-md transition-all ${
                        ...
                        >
                            ...
                            <span className={`absolute -top-1 -right-1 bg-indigo-500 text-white font-mono text-[9px] font-bold min-w-4 h-4 px-1 rounded-full flex items-center justify-center border border-slate-900 shadow-md transition-all ${
                        ...
                        {auth.user ? (
                            <div id="profile-dropdown-container" className="relative shrink-0">
                                ...
                                >
                                    ...
                                    <span className="max-w-[100px] truncate hidden sm:inline text-xs font-semibold">{auth.user.name}</span>
                                ...
                                {showProfileDropdown && (
                                    <div className="absolute right-0 mt-2 w-56 bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl py-2 z-50 text-slate-200">
                                        <div className="px-4 py-2.5 border-b border-slate-700/60">
                                            <p className="text-[9px] text-cyan-400 font-bold uppercase tracking-wider">Klien Terdaftar</p>
                                            <p className="text-xs font-bold text-white truncate mt-0.5">{auth.user.name}</p>
                                            <p className="text-[10px] text-slate-400 truncate font-mono mt-0.5">{auth.user.email}</p>
                                        </div>

                                        {auth.user.is_admin && (
                                            <Link
                                                href={route('admin.dashboard')}
                                                className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold text-cyan-400 hover:bg-slate-700/80 transition-colors text-left"
                                            >
                                        ...
                                        <Link
                                            ...
                                            className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-700/80 transition-colors text-left"
                                        >
                                        ...
                                        <Link
                                            ...
                                            method="post"
                                            as="button"
                                            className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-semibold text-rose-400 hover:bg-rose-950/40 transition-colors text-left border-t border-slate-700/60 mt-1 pt-2"
                                        >
                ...
                <div className="bg-[#070B14] border-t border-slate-800/80 text-xs w-full">
                    <div className="max-w-7xl mx-auto px-4 md:px-8 py-2.5 flex items-center justify-between overflow-x-auto whitespace-nowrap gap-6 scrollbar-none">
                        <div className="flex items-center gap-6 font-semibold text-slate-300">
                            ...
                            <Link href={route('jasa-pembuatan')} className="px-3 py-1 bg-gradient-to-r from-indigo-600/30 to-cyan-500/30 border border-cyan-500/40 rounded-full text-cyan-300 font-bold flex items-center gap-1.5 hover:border-cyan-400 transition-all shadow-sm">
                                <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping" />
                                Jasa Website & Android
                            </Link>
                            <Link href={route('katalog')} className="hover:text-white transition-colors">
                                Katalog Paket
                            </Link>
                            <Link href={route('cara-pemesanan')} className="hover:text-white transition-colors">
                                Cara Pemesanan
                            </Link>
                            <Link href={route('tentang-kami')} className="hover:text-white transition-colors">
                                Tentang Kami
                            </Link>
                            <Link href={route('hubungi-kami')} className="hover:text-white transition-colors">
                                Hubungi Kami
                            </Link>
                        </div>
                        <a 
                            ...
                            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3.5 py-1.5 rounded-full text-[11px] flex items-center gap-1.5 transition-all shadow-md shadow-emerald-600/25 shrink-0"
                        >
                    ...
                    <div className="md:hidden bg-slate-900 border-b border-slate-800 px-4 py-4 space-y-3">
                        <form onSubmit={handleSearchSubmit} className="relative w-full">
                            <input
                                ...
                                className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2 text-xs"
                            />
                        </form>
                        <div className="grid grid-cols-2 gap-2 text-xs font-semibold">
                            <Link href={route('home')} className="p-2.5 rounded-xl bg-slate-800/60 text-slate-200">Beranda</Link>
                            <Link href={route('jasa-pembuatan')} className="p-2.5 rounded-xl bg-indigo-900/40 text-cyan-300 font-bold border border-indigo-700/50">Jasa Web & Android</Link>
                            <Link href={route('katalog')} className="p-2.5 rounded-xl bg-slate-800/60 text-slate-200">Katalog Paket</Link>
                            <Link href={route('cara-pemesanan')} className="p-2.5 rounded-xl bg-slate-800/60 text-slate-200">Cara Pemesanan</Link>
            ...
            <footer className="border-t border-slate-800 bg-[#060913] py-14 px-6 md:px-12 text-slate-400">
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            />
                            <div>
                                <h4 className="font-extrabold text-lg text-white leading-tight">{storeName}</h4>
                                <p className="text-[10px] text-cyan-400 font-semibold tracking-wider uppercase">Web & Android Software House</p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        <h4 className="font-bold text-xs text-white uppercase tracking-wider">Layanan Utama</h4>
                        <ul className="space-y-2.5 text-xs">
                            <li><Link href={route('katalog')} className="hover:text-indigo-400 transition-colors">Jasa Pembuatan Website</Link></li>
                            <li><Link href={route('katalog')} className="hover:text-indigo-400 transition-colors">Jasa Aplikasi Android (Flutter)</Link></li>
                            <li><Link href={route('jasa-pembuatan')} className="hover:text-cyan-400 text-cyan-300 font-bold transition-colors">Bundle Website + Mobile App</Link></li>
                            <li><Link href={route('katalog')} className="hover:text-indigo-400 transition-colors">Desain UI/UX & Prototype</Link></li>
                            <li><Link href={route('katalog')} className="hover:text-indigo-400 transition-colors">Setup Cloud VPS & Maintenance</Link></li>
                        </ul>
                    </div>

                    {/* Column 3: Informasi & Kebijakan */}
                    <div className="space-y-3">
                        <h4 className="font-bold text-xs text-white uppercase tracking-wider">Pusat Bantuan</h4>
                        <ul className="space-y-2.5 text-xs">
                            <li><Link href={route('cara-pemesanan')} className="hover:text-indigo-400 transition-colors">Cara Pemesanan Jasa</Link></li>
                            <li><Link href={route('tentang-kami')} className="hover:text-indigo-400 transition-colors">Tentang Kami</Link></li>
                            <li><Link href={route('hubungi-kami')} className="hover:text-indigo-400 transition-colors">Hubungi Konsultan Kami</Link></li>
                            <li><Link href={route('syarat-ketentuan')} className="hover:text-indigo-400 transition-colors">Syarat & Ketentuan Layanan</Link></li>
                            <li><Link href={route('kebijakan-privasi')} className="hover:text-indigo-400 transition-colors">Kebijakan Privasi</Link></li>
                        </ul>
                    </div>

                    {/* Column 4: Kontak & Social Media */}
                    <div className="space-y-4">
                        <h4 className="font-bold text-xs text-white uppercase tracking-wider">Hubungi Kami</h4>
                        ...
                        <div className="flex flex-col gap-2 text-xs">
                            ...
                            <span className="text-[11px] text-slate-500">Jam Operasional: 08.00 - 22.00 WIB</span>
                        </div>
                    </div>
                </div>

                {/* Bottom Copyright */}
                <div className="max-w-7xl mx-auto border-t border-slate-800/80 mt-10 pt-6 text-center">
